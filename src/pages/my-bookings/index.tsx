  ...
  return (
    <View layout}}
      ...
      <View className="flex items-start gap-3">
        <View className="w-12 h-12 rounded-xl flex items-center justify-center font-bold text-lg flex-shrink-0"
        ...
        <View className="flex-1 min-w-0">
          <View className="flex items-start justify-between gap-2 mb-1">
            <View>
              <Text className="text-sm font-bold text-[#2C2420]">{c?.displayName ?? "咨询师"}</Text>
              {c?.counselorTypes && (c.counselorTypes as string[]).length > 0 && (
                <Text className="text-xs text-[#9B8E82] mt-0.5">{(c.counselorTypes as string[]).join(" · ")}</Text>
              )}
            </View>
            <Text className="text-[11px] font-medium px-2 py-0.5 rounded-full flex-shrink-0"
              style={{ color: badge.color, background: badge.bg }}>{badge.label}</Text>
          </View>
          {dt && (
            <View className="flex items-center gap-3 text-xs text-[#9B8E82] mt-1.5">
          ...
          <View className="flex items-center justify-between mt-2">
            <Text className="text-xs text-[#9B8E82] flex items-center gap-1">
              <Video className="w-3.5 h-3.5" />{b.durationMinutes} 分钟 · {b.sessionMode}
            </Text>
            <Text className="text-sm font-bold text-[#2C2420]">¥{b.priceAmount}</Text>
          </View>
        </View>
      </View>
      {b.status === "pending_confirmation" && (
        <View className="w-full mt-3 py-2.5 rounded-xl text-sm font-semibold border"
          style={{ borderColor: "#E0D8CE", color: "#9B8E82" }}>取消预约</View>
      )}
      {b.status === "pending_payment" && (
        <View className="flex gap-2 mt-3">
          <View className="flex-1 py-2.5 rounded-xl text-sm font-semibold border flex items-center justify-center gap-1"
            style={{ borderColor: "#9CB48A", color: "#9CB48A" }}>
            <Text>💬</Text>私信咨询师</View>
          <View className="flex-[2] py-2.5 rounded-xl text-white text-sm font-semibold"
            style={{ background: "#9CB48A" }}>立即支付 ¥{b.priceAmount}</View>
        </View>
      )}
      {b.status === "paid" && (
        <View className="flex gap-2 mt-3">
          <View className="flex-1 py-2.5 rounded-xl text-sm font-semibold border flex items-center justify-center gap-1"
            style={{ borderColor: "#9CB48A", color: "#9CB48A" }}>
            <Text>💬</Text>私信咨询师</View>
          <View className="flex-1 py-2.5 rounded-xl text-sm font-semibold border"
            style={{ borderColor: "#E0D8CE", color: "#9B8E82" }}>申请改期</View>
        </View>
      )}
      {b.status === "completed" && (
        <View className="w-full mt-3 py-2.5 rounded-xl text-sm font-semibold border"
          style={{ borderColor: "#E0D8CE", color: "#9B8E82" }}>写咨询反馈</View>
      )}
    </View>
  );
}
...
export default function MyBookingsScreen() {
  ...
  useEffect(() => {
    if (!user) return;
    request("/api/bookings/my").then(d => {
  ...
  return (
    <View className="h-svh flex flex-col" style={{ background: "var(--color-surface)" }}>
      <View className="flex-none sticky top-0 px-5 pb-0 z-20"
        style={{ background: "#F5F0E8", borderBottom: "1px solid #EBE7DF", paddingTop: 14 }}>
        <View className="flex items-center gap-3 mb-3">
          <View onClick={() => window.history.back()}
          ...
          <Text className="text-base font-bold" style={{ color: "#2C2420" }}>全部预约</Text>
        </View>
        <Text className="text-xl font-bold text-[#2C2420] mb-4">我的预约</Text>
        <View className="flex gap-1.5 pb-3 border-b border-[#EBE7DF]">
          {TABS.map(t => (
            <View key={t.key} onClick={() => setTab(t.key)}
              ...
              {counts[t.key] > 0 && tab !== t.key && (
                <Text className="absolute -top-1.5 -right-0.5 w-4 h-4 rounded-full text-[10px] flex items-center justify-center text-white"
                  style={{ background: "#E07B54" }}>{counts[t.key]}</Text>
              )}
            </View>
          ))}
        </View>
      </View>
      <View className="flex-1 overflow-y-auto px-4 pt-4 pb-28">
        {loading ? (
          <View className="space-y-3">{[1,2].map(i => <View key={i} className="h-28 rounded-2xl skeleton" />)}</View>
        ) : displayed.length === 0 ? (
          <View className="text-center py-20">
            <Calendar className="w-10 h-10 mx-auto mb-3" style={{ color: "#C4BDB5" }} />
            <Text className="text-sm text-[#9B8E82]">暂无记录</Text>
          </View>
        ) : (
          <View>
